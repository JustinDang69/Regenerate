/* Tiny static server for reviewing round-2 artifacts locally. Isolated from the
   Next.js app — serves this folder only, on port 4322. */
...
createServer(async (req, res) => {
  try {
    ...
    const file = join(ROOT, normalize(url === "/" ? "/round-3/index.html" : url));
    if (!file.startsWith(ROOT)) { res.writeHead(403).end(); return; }
    const body = await readFile(file);
    res.writeHead(200, { "content-type": TYPES[extname(file)] || "application/octet-stream", "cache-control": "no-store" });
    res.end(body);
  } catch { res.writeHead(404).end("not found"); }
}).listen(4322, () => console.log("review server on http://localhost:4321"));
